...
No account. No telemetry. No network calls. Five permissions.
...
**In development — nothing is installable yet.**

| Phase | Scope | Status |
|---|---|---|
| P1 | Calm surface — clock, The Line, pins, themes, wallpaper, backups | In progress |
...
| **Trustworthy** | Five install-time permissions: `storage`, `unlimitedStorage`, `favicon`, `commands`, `contextMenus`. Everything else is optional and requested in context, with a reason, the first time you use the feature that needs it. No account, no telemetry, no network requests. Fonts and icons are local. |
